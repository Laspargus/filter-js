...
// C'est le tableau des mots clefs déjà sélectionné
let currentKeywords = [];

//C'est les mots clefs suggérés
let proposedKeywords = [];

//tous les mots clefs
const allKeywords = [];

//les mots clefs de la même famille
...
similarKeywords = (keywordTapped) => {
  const sameCatKeywords = [];
  keywordsCategories.forEach((categorie) => {
    categorie.keywords.forEach((keyword) => {
      if (keywordTapped === keyword) {
        sameCatKeywords.push(categorie.keywords);
      }
    });
  });

  sameCatKeywordsWithoutEntry = sameCatKeywords[0].filter((key) => {
    return key != keywordTapped;
  });

  return sameCatKeywordsWithoutEntry;
};
...
const cleanedKeyword = (keyword) => {
  console.log("avant de cleaner", keyword);
  let cleanedKeyword = keyword.toLowerCase().replace(/[^a-zA-Z0-9]/g, "");
...
const showKeywordsList = (value) => {
  ...
  if (value.length >= 3) {
    ...
    // de value on veut récupérer keyword
    valueUpperCase = value.charAt(0).toUpperCase() + value.slice(1);

    // Keyword en camel
    const keyword = allKeywords.filter((keyword) =>
      keyword.includes(valueUpperCase)
    )[0];
    console.log(keyword);
    console.log("la meme cat ", similarKeywords(keyword));

    if (!keywords.includes(cleanedKeyword(keyword))) {
    ...
    console.log(similarKeywords(keyword));
    similarKeywords(keyword).forEach((keyword) => {
      console.log(keyword);
      keyWordUl.innerHTML += `
       <li onclick="addNewKeyword('${keyword}', '${cleanedKeyword(
        keyword
